...
reader.on('close', () => {
    ...
    const crenz_answers: number[][] = []
    for (let i=1;i <= n; i++) {
        const answer: string[] = lines[i].split(' ')
        const crenz_answer: number[] = answer.map((ans) => {
...
const transpose = ((crenz_answers: number[][]): number[][] => {
    const results: number[][] = []
    crenz_answers[0].map((_,c) => {
        let transposed_answers: number[] = crenz_answers.map((r) => r[c])
